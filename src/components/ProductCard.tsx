...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <div
      className={`group relative transition-all duration-300 ${outOfStock ? 'opacity-60 cursor-not-allowed' : ''}`}
      ...
      <Link to={`/product/${product.id}`}>
        <div className="relative overflow-hidden rounded-lg aspect-square bg-gray-100 mb-3">
          ...
          {!outOfStock && (
            ...
            >
              ...
              >
                <Heart size={16} className={isInFavorites(product.id) ? 'fill-current' : ''} />
              </button>
            </div>
          )}

          {/* View Product hover effect */}
          {!outOfStock && (
            <div
        ...
        <h3
          className={`font-medium ${
            outOfStock ? 'text-gray-500' : 'text-gray-800 group-hover:text-terracotta-600 transition-colors'
          }`}
        >
          {product.name}
        </h3>
        ...
        <div className="mt-1 flex justify-between items-center">
          <span className="text-terracotta-600 font-medium">{formatPrice(variant.price)}</span>
